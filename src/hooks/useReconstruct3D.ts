  ...
  const reconstruct = useCallback(
    async (
      ...
      try {
        ...
        const texturePrompt = reconstructOptions.manualTexturePrompt;
        ...
        const response = await reconstruct3D(
          ...
          // Progress callback - map Meshy's 0-100 to our 20-80 range
          (meshyProgress, meshyStatus) => {
            const mappedProgress = 20 + (meshyProgress * 0.6);
